...
@property (nonatomic, strong) SKScrollView *scrollView;
...
- (id)initWithSectionTitles:(NSArray *)sectiontitles {
    ...
    if (self) {
        ...
        self.type = SKSegmentedControlTypeText;
    }
...
- (id)initWithSectionImages:(NSArray*)sectionImages sectionSelectedImages:(NSArray*)sectionSelectedImages {
    ...
    if (self) {
        ...
        self.type = SKSegmentedControlTypeImages;
    }
...
- (instancetype)initWithSectionImages:(NSArray *)sectionImages sectionSelectedImages:(NSArray *)sectionSelectedImages titlesForSections:(NSArray *)sectiontitles {
    ...
    if (self) {
        ...
        self.sectionSelectedImages = sectionSelectedImages;
		self.sectionTitles = sectiontitles;
        self.type = SKSegmentedControlTypeTextImages;
    }
...
- (void)commonInit {
    self.scrollView = [[SKScrollView alloc] init];
    ...
    self.selectionStyle = SKSegmentedControlSelectionStyleTextWidthStripe;
    self.selectionIndicatorLocation = SKSegmentedControlSelectionIndicatorLocationUp;
    self.segmentWidthStyle = SKSegmentedControlSegmentWidthStyleFixed;
...
- (void)setSelectionIndicatorLocation:(SKSegmentedControlSelectionIndicatorLocation)selectionIndicatorLocation {
	_selectionIndicatorLocation = selectionIndicatorLocation;
	
	if (selectionIndicatorLocation == SKSegmentedControlSelectionIndicatorLocationNone) {
...
- (void)setSegmentWidthStyle:(SKSegmentedControlSegmentWidthStyle)segmentWidthStyle {
    // Force SKSegmentedControlSegmentWidthStyleFixed when type is SKSegmentedControlTypeImages.
    if (self.type == SKSegmentedControlTypeImages) {
        _segmentWidthStyle = SKSegmentedControlSegmentWidthStyleFixed;
    } else {
        _segmentWidthStyle = segmentWidthStyle;
    }
}

- (void)setBorderType:(SKSegmentedControlBorderType)borderType {
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == SKSegmentedControlTypeText) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            BOOL locationUp = (self.selectionIndicatorLocation == SKSegmentedControlSelectionIndicatorLocationUp);
            BOOL selectionStyleNotBox = (self.selectionStyle != SKSegmentedControlSelectionStyleBox);
            ...
            CGRect rect;
            if (self.segmentWidthStyle == SKSegmentedControlSegmentWidthStyleFixed) {
...
//                rectDiv = CGRectMake((self.segmentWidth * idx) - (self.verticalDividerWidth / 2), self.selectionIndicatorHeight * 2, self.verticalDividerWidth, self.frame.size.height - (self.selectionIndicatorHeight * 4));
            ...
            } else if (self.segmentWidthStyle == SKSegmentedControlSegmentWidthStyleDynamic) {
...
//                verticalDividerLayer.frame = rectDiv;
    ...
    } else if (self.type == SKSegmentedControlTypeImages) {
        [self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            UIImage *icon = iconImage;
            CGFloat imageWidth = icon.size.width;
            CGFloat imageHeight = icon.size.height;
            CGFloat y = roundf(CGRectGetHeight(self.frame) - self.selectionIndicatorHeight) / 2 - imageHeight / 2 + ((self.selectionIndicatorLocation == SKSegmentedControlSelectionIndicatorLocationUp) ? self.selectionIndicatorHeight : 0);
    ...
    } else if (self.type == SKSegmentedControlTypeTextImages){
		[self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            UIImage *icon = iconImage;
            CGFloat imageWidth = icon.size.width;
            CGFloat imageHeight = icon.size.height;
    ...
    if (self.selectedSegmentIndex != SKSegmentedControlNoSegment) {
        if (self.selectionStyle == SKSegmentedControlSelectionStyleArrow) {
            ...
            if (!self.selectionIndicatorStripLayer.superlayer) {
                ...
                if (self.selectionStyle == SKSegmentedControlSelectionStyleBox && !self.selectionIndicatorBoxLayer.superlayer) {
...
- (void)addBackgroundAndBorderLayerWithRect:(CGRect)fullRect {
    ...
    // Border layer
    if (self.borderType & SKSegmentedControlBorderTypeTop) {
...
- (void)setArrowFrame {
    ...
    if (self.selectionIndicatorLocation == SKSegmentedControlSelectionIndicatorLocationDown) {
    ...
    if (self.selectionIndicatorLocation == SKSegmentedControlSelectionIndicatorLocationUp) {
...
- (CGRect)frameForSelectionIndicator {
    ...
    if (self.selectionStyle == SKSegmentedControlSelectionStyleArrow) {
        ...
        if (self.selectionStyle == SKSegmentedControlSelectionStyleTextWidthStripe &&
            sectionWidth <= self.segmentWidth &&
            self.segmentWidthStyle != SKSegmentedControlSegmentWidthStyleDynamic) {
            CGFloat widthToEndOfSelectedSegment = (self.segmentWidth * self.selectedSegmentIndex) + self.segmentWidth;
            CGFloat widthToStartOfSelectedIndex = (self.segmentWidth * self.selectedSegmentIndex);
            ...
            if (self.segmentWidthStyle == SKSegmentedControlSegmentWidthStyleDynamic) {
...
- (void)updateSegmentsRects {
    ...
    if (self.type == SKSegmentedControlTypeText && self.segmentWidthStyle == SKSegmentedControlSegmentWidthStyleFixed) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            CGFloat stringWidth = [self measureTitleAtIndex:idx].width + self.segmentEdgeInset.left + self.segmentEdgeInset.right;
            self.segmentWidth = MAX(stringWidth, self.segmentWidth);
        }];
    } else if (self.type == SKSegmentedControlTypeText && self.segmentWidthStyle == SKSegmentedControlSegmentWidthStyleDynamic) {
    ...
    } else if (self.type == SKSegmentedControlTypeTextImages && self.segmentWidthStyle == SKSegmentedControlSegmentWidthStyleFixed){
        //lets just use the title.. we will assume it is wider then images...
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            CGFloat stringWidth = [self measureTitleAtIndex:idx].width + self.segmentEdgeInset.left + self.segmentEdgeInset.right;
            self.segmentWidth = MAX(stringWidth, self.segmentWidth);
        }];
    } else if (self.type == SKSegmentedControlTypeTextImages && SKSegmentedControlSegmentWidthStyleDynamic) {
        NSMutableArray *mutableSegmentWidths = [NSMutableArray array];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (CGRectContainsPoint(self.bounds, touchLocation)) {
        ...
        } else if (self.type == SKSegmentedControlTypeTextImages || self.type == SKSegmentedControlTypeText) {
...
//- (void)setIndicatorColour:(UIColor*)colour {
//    self.selectionIndicatorStripLayer.backgroundColor = self.selectionIndicatorColor.CGColor;
//}

- (void)resetIndicatorColour {
    self.selectionIndicatorStripLayer.backgroundColor = self.selectionIndicatorColor.CGColor;
}
...
- (void)setSelectedSegmentIndex:(NSUInteger)index animated:(BOOL)animated notify:(BOOL)notify {
    ...
    if (index == SKSegmentedControlNoSegment) {
        ...
        if (animated) {
            ...
            if(self.selectionStyle == SKSegmentedControlSelectionStyleArrow) {
                ...
                if ([self.selectionIndicatorStripLayer superlayer] == nil) {
                    ...
                    if (self.selectionStyle == SKSegmentedControlSelectionStyleBox && [self.selectionIndicatorBoxLayer superlayer] == nil)
                        [self.scrollView.layer insertSublayer:self.selectionIndicatorBoxLayer atIndex:0];
